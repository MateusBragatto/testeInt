...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            //alterando pra ambos trafegos irem pela porta 0

            if(hdr.ipv4.dstAddr == 0xc0a8320C){
                standard_metadata.priority = (bit<3>) 7;
                standard_metadata.qid = 1;
            }
            else if(hdr.ipv4.dstAddr == 0xc0a8320C){
                standard_metadata.priority = (bit<3>) 0;
                standard_metadata.qid = 2;
            }
